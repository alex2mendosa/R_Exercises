...
first 2 and the last 2 characters of words from a given sentence
y<-"There are some things 
that are so unforgivable that they make other things easily forgivable."
n<-nchar(y)
...
#6+ Write R program to get a sentence from a given y
...
#7+ Write R program to get a string from a given y
...
and last characters have been swapped 
...
#15+
#Write R
program to remove characters with odd
indices from y.
...
#18+
Remove leading and trailing whitespace from strings x1.
...
#24+
Write R code to insert a string ins_str
...
of last characters of each word of sentece y. 
...
#28+
Write R code to convert all vowels to uppercase vowels in z
...
#31+
#Write R program to check if a string contains all letters of the alphabet
...
#34+
#Replace word consectetur in x to its reverse version
...
#36+
#WriteR program to remove all except first appearance of duplicated words
#from a given string song_1

words<-strsplit(song_1,split="\\W+")[[1]]
words_count<-data.frame(table(words)) %>% filter(Freq>1) %>%
  pull(words) %>% as.character()

song_2<-strsplit(  gsub("\n","",song_1) ,split=" ")

for (i in 1:length(words_count)) {
    n<-nchar(words_count[i])
    loc<-gregexpr( words_count[i],song_2 )[[1]][-1]
