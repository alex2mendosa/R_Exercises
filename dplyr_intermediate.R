...
Lets have a look on mtcars data frame
build in R.
Type mtcars in console.
?mtcars command provides description of mtcars columns.

db_1<-mtcars[,1:4] we will use only first 4 columns
With mutate() function we can 
add new column to data frame by specifying 
...
create column with single value, on the other hand
...
is greater than 25% of observations in Sell_Out.
...
Result is "Even" if value in Quantity_Sold column is even and 
"Uneven" otherwise. 
sample_1$Quantity_Sold %% 2 ==0 as argument of ifelse()
simply repsesents logical vector of same length as
...
therefore more than 1 condition is possible
...
Months in sample_1 are indicated as month indices.
...
switch() chooses among several alternatives and provides 
output as soon as sample_element fits listed conditions.

Note 
sample_element<-c("Japan","USA","United Kingdom")
switch(sample_element,"USA"="Washington" , "United Kingdom"="London", "Japan"="Tokyo")
produces error as first element must be singe value.
...
Exercise requres prior experience with sapply()  function.

Solution
mutate(sample_1,switch(Month,"1"="Jan","2"="Feb","3"="Mar",
                       "4"="Apr","5"="May","6"="Jun","7"="Jul") )
will not work as switch() can accept only vector of length 1, 
while sample_1$Month contains 7 elements.
...
which allows switch() function to run.
...
With single mutate() we created 2 columns where column Status 
...
months before Stock would become 0  assuming constant Sell_Out  per month
...
which is already present in base R version.
With dplyr it is possible to aggregate data 
into single value providing summary of 
...
which requres explicit specification of list of grouping elements.
...
sample_1 is data frame with 8 different SKU and sales history for 2 months. 
...
were average sales are grouped[organised] by Business and
SKU. It would require grouping by 2
variables.

sample_1<-group_by(sample_1,Business,SKU)
summarise(sample_1,mean(Sell_Out) )

As Output we get average Sales per SKU 
sorted by Group.
...
Now group data by Busines and month to estimate mean() and total [sum] Sell Out
