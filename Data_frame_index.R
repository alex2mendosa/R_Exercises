...
column 2 and all rows from a data frame using
...
# however when combine in data frame character class is 
# retransformed to factor class.
...
next you indoicate vector with column  numbers. 
If row of column index is not mentioned,
...
first and second columns using names
...
use str() again to check if column naw have different class.
...
second and fifth row with, 1-st and 4-th row where
infices are referenced in numeric vector. 

sample_frame[c(2,5),]
sample_frame[c(1,4),]

Notice, we indicate rows, commo,
clunns are not referenced, therefore 
...
indicated total number of columns. 

Write 2  R expressiom to extract 
first and last columns, second and penultimate column. Use length fucntion 
to estimate indices of columns.

sample_frame[,c(1,length(sample_frame) )]
sample_frame[,c(2,length(sample_frame)-1 )]

Because we did not indocate specific rows, 
R reads as as instruction to eactract all rows. 
...
Use $ to access columns of each data frame,
and use them as argument of data frame fucntion to binf 
vectors in the following order

digits,y_logic,case_low,case_high

forming a data frame called sample_frame_3

Next use name fcuntion to rename columns a new composed data
...
element in 4-th row 2-nd column
...
use seq() fucntion to create vector of uneneb numbers abd use it to extract 
uneven rows from colimn 1 and 3
...
use seq() fucntion to create vector of even  numbers abd use it to extract 
even rows from all columns, coluns shopuld be in reverse order
